...
func setupAuthTestDB(t *testing.T) (*authTestContext, func()) {
	...
	_, err = db.Exec(`
		INSERT INTO auth.tenant_domains (tenant_id, domain, created_at)
		VALUES ($1, $2, NOW())
	`, tenantID, testDomain)
	require.NoError(t, err)
	...
	cleanup := func() {
		db.Exec("DELETE FROM auth.tenant_oidc_configs WHERE tenant_id = $1", tenantID)
		db.Exec("DELETE FROM auth.tenant_domains WHERE tenant_id = $1", tenantID)
		db.Exec("DELETE FROM auth.tenants WHERE id = $1", tenantID)
		db.Close()
	}
