...
func (p *StrategyPillarCacheProjector) handleConfigurationCreated(ctx context.Context, eventData []byte) error {
	...
	if err := json.Unmarshal(eventData, &event); err != nil {
		wrappedErr := fmt.Errorf("unmarshal MetaModelConfigurationCreated event data in strategy pillar cache projector: %w", err)
	...
	for _, pillar := range event.Pillars {
		...
		if err := p.readModel.Insert(ctx, dto); err != nil {
			return fmt.Errorf("project MetaModelConfigurationCreated strategy pillar cache insert for pillar %s: %w", pillar.ID, err)
		}
	}
...
func (p *StrategyPillarCacheProjector) handlePillarAdded(ctx context.Context, eventData []byte) error {
	...
	if err := p.readModel.Insert(ctx, dto); err != nil {
		return fmt.Errorf("project StrategyPillarAdded strategy pillar cache insert for pillar %s: %w", event.PillarID, err)
	}
	return nil
}
...
func (p *StrategyPillarCacheProjector) handlePillarRemoved(ctx context.Context, eventData []byte) error {
	...
	if err := p.readModel.Delete(ctx, event.PillarID); err != nil {
		return fmt.Errorf("project StrategyPillarRemoved strategy pillar cache delete for pillar %s: %w", event.PillarID, err)
	}
	return nil
}
...
func (p *StrategyPillarCacheProjector) unmarshalAndUpdate(ctx context.Context, eventData []byte, mutate func(pillarEvent, *readmodels.StrategyPillarCacheDTO)) error {
	var event pillarEvent
	if err := json.Unmarshal(eventData, &event); err != nil {
		wrappedErr := fmt.Errorf("unmarshal strategy pillar event data in cache projector: %w", err)
	...
	if err != nil {
		...
		return fmt.Errorf("load active strategy pillar %s for cache mutation: %w", event.PillarID, err)
	}
	...
	if err := p.readModel.Insert(ctx, *existing); err != nil {
		return fmt.Errorf("upsert strategy pillar cache entry for pillar %s: %w", event.PillarID, err)
	}
	return nil
}
