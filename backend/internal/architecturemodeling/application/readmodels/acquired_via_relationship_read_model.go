...
func (rm *AcquiredViaRelationshipReadModel) Upsert(ctx context.Context, dto AcquiredViaRelationshipDTO) error {
	...
	_, err = rm.db.ExecContext(ctx,
		`INSERT INTO architecturemodeling.acquired_via_relationships (id, tenant_id, acquired_entity_id, component_id, notes, created_at)
		 VALUES ($1, $2, $3, $4, $5, $6)
...
func (rm *AcquiredViaRelationshipReadModel) UpdateNotesByComponentID(ctx context.Context, componentID string, notes string) error {
	return rm.execWithTenant(ctx,
		`UPDATE architecturemodeling.acquired_via_relationships SET notes = $1 WHERE tenant_id = $2 AND component_id = $3`,
...
func (rm *AcquiredViaRelationshipReadModel) DeleteByComponentID(ctx context.Context, componentID string) error {
	return rm.execWithTenant(ctx,
		`DELETE FROM architecturemodeling.acquired_via_relationships WHERE tenant_id = $1 AND component_id = $2`,
...
func (rm *AcquiredViaRelationshipReadModel) MarkAsDeleted(ctx context.Context, id string, deletedAt time.Time) error {
	...
	_, err = rm.db.ExecContext(ctx,
		"UPDATE architecturemodeling.acquired_via_relationships SET is_deleted = TRUE, deleted_at = $1 WHERE tenant_id = $2 AND id = $3",
	...
	err = rm.db.WithReadOnlyTx(ctx, func(tx *sql.Tx) error {
		err := tx.QueryRowContext(ctx,
			`SELECT r.id, r.acquired_entity_id, COALESCE(ae.name, '') as acquired_entity_name,
			 ...
			 LEFT JOIN architecturemodeling.acquired_entities ae ON ae.tenant_id = r.tenant_id AND ae.id = r.acquired_entity_id AND ae.is_deleted = FALSE
