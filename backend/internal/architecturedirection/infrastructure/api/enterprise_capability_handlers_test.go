...
type mockECCommandBus struct {
	dispatchedCommands []cqrs.Command
	dispatchErr        error
	createdID          string
}

func (m *mockECCommandBus) Dispatch(ctx context.Context, cmd cqrs.Command) (cqrs.CommandResult, error) {
...
func (m *mockECCommandBus) Register(commandName string, handler cqrs.CommandHandler) {}
