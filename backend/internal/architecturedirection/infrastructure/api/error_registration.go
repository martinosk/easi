...
func init() {
	...
	registry.RegisterValidation(sharedvo.ErrDescriptionTooLong, "Text cannot exceed 1000 characters")
	...
	registry.RegisterValidation(aggregates.ErrJourneyTargetMaturityOnNonMaturity, "A target maturity is only valid for maturity journeys")
	registry.RegisterValidation(aggregates.ErrJourneyMaturityRequiresTarget, "A maturity journey requires a target maturity")
	registry.RegisterValidation(aggregates.ErrJourneyMaturityRefusesApplications, "A maturity journey carries no applications")
	registry.RegisterValidation(aggregates.ErrJourneyMaturityTargetNotAboveCurrent, "The target maturity must be above the capability's current maturity")
	registry.RegisterValidation(valueobjects.ErrTargetMaturityOutOfRange, "Target maturity must be between 0 and 99")
	registry.RegisterValidation(valueobjects.ErrInvalidJourneyKind, "Kind must be one of migration, consolidation, carve-out, move, maturity")
