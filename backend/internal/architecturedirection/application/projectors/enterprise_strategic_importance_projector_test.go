...
type mockEnterpriseStrategicImportanceReadModel struct {
	...
	updates      []readmodels.UpdateImportanceParams
	deletedIDs   []string
	insertErr    error
	updateErr    error
	deleteErr    error
...
func (m *mockEnterpriseStrategicImportanceReadModel) Update(ctx context.Context, params readmodels.UpdateImportanceParams) error {
	if m.updateErr != nil {
		return m.updateErr
	}
