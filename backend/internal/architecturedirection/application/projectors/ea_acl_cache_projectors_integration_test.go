...
func setupACLCacheTest(t *testing.T) *aclCacheTestFixture {
	...
	t.Cleanup(func() {
		db.Exec("DELETE FROM architecturedirection.ea_realization_cache WHERE tenant_id = $1", aclTestTenant)
		...
		db.Exec("DELETE FROM architecturedirection.domain_capability_metadata WHERE tenant_id = $1", aclTestTenant)
		db.Close()
	})
...
func (f *aclCacheTestFixture) countRealizationsByColumn(column, value string) int {
	...
	var count int
	err := f.db.QueryRow(
		fmt.Sprintf("SELECT COUNT(*) FROM architecturedirection.ea_realization_cache WHERE tenant_id = $1 AND %s = $2", column),
		aclTestTenant, value,
	).Scan(&count)
	require.NoError(f.t, err)
	return count
...
func TestRealizationCacheProjector_ComponentUpdated_UpdatesCachedName(t *testing.T) {
	...
	var name string
	err = f.db.QueryRow(
		"SELECT component_name FROM architecturedirection.ea_realization_cache WHERE tenant_id = $1 AND component_id = $2",
...
func TestMetadataProjector_CapabilityMetadataUpdated_UpdatesMaturity(t *testing.T) {
	...
	err = f.db.QueryRow(
		"SELECT maturity_value FROM architecturedirection.domain_capability_metadata WHERE tenant_id = $1 AND capability_id = $2",
