...
func (rm *EAFitScoreCacheReadModel) Upsert(ctx context.Context, entry FitScoreEntry) error {
	return rm.execForTenant(ctx,
		`INSERT INTO architecturedirection.ea_fit_score_cache (tenant_id, component_id, pillar_id, score, rationale)
		 VALUES ($1, $2, $3, $4, $5)
...
func (rm *EAFitScoreCacheReadModel) Delete(ctx context.Context, componentID, pillarID string) error {
	return rm.execForTenant(ctx,
		"DELETE FROM architecturedirection.ea_fit_score_cache WHERE tenant_id = $1 AND component_id = $2 AND pillar_id = $3",
