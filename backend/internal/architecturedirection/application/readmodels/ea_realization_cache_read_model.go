...
func (rm *EARealizationCacheReadModel) Upsert(ctx context.Context, entry RealizationEntry) error {
	return rm.execForTenant(ctx,
		`INSERT INTO architecturedirection.ea_realization_cache (tenant_id, realization_id, capability_id, component_id, component_name, origin)
		 VALUES ($1, $2, $3, $4, $5, $6)
...
func (rm *EARealizationCacheReadModel) Delete(ctx context.Context, realizationID string) error {
	return rm.execForTenant(ctx,
		"DELETE FROM architecturedirection.ea_realization_cache WHERE tenant_id = $1 AND realization_id = $2",
		realizationID,
	)
}
...
func (rm *EARealizationCacheReadModel) DeleteByCapabilityID(ctx context.Context, capabilityID string) error {
	return rm.execForTenant(ctx,
		"DELETE FROM architecturedirection.ea_realization_cache WHERE tenant_id = $1 AND capability_id = $2",
		capabilityID,
	)
}
...
func (rm *EARealizationCacheReadModel) UpdateComponentName(ctx context.Context, componentID, componentName string) error {
	return rm.execForTenant(ctx,
		"UPDATE architecturedirection.ea_realization_cache SET component_name = $2 WHERE tenant_id = $1 AND component_id = $3",
