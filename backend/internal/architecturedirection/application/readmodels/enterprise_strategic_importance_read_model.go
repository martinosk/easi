...
func (rm *EnterpriseStrategicImportanceReadModel) Insert(ctx context.Context, dto EnterpriseStrategicImportanceDTO) error {
	...
	_, err = rm.db.ExecContext(ctx,
		"DELETE FROM architecturedirection.enterprise_strategic_importance WHERE tenant_id = $1 AND id = $2",
	...
	_, err = rm.db.ExecContext(ctx,
		`INSERT INTO architecturedirection.enterprise_strategic_importance
...
type UpdateImportanceParams struct {
	ID         string
	Importance int
	Rationale  string
}

func (rm *EnterpriseStrategicImportanceReadModel) Update(ctx context.Context, params UpdateImportanceParams) error {
	tenantID, err := sharedctx.GetTenant(ctx)
	if err != nil {
		return err
	}
	...
	_, err = rm.db.ExecContext(ctx,
		`UPDATE architecturedirection.enterprise_strategic_importance SET importance = $1, rationale = $2, updated_at = CURRENT_TIMESTAMP
		 WHERE tenant_id = $3 AND id = $4`,
		params.Importance, params.Rationale, tenantID.Value(), params.ID,
	)
	return err
}
	...
	err = rm.db.WithReadOnlyTx(ctx, func(tx *sql.Tx) error {
		rows, err := tx.QueryContext(ctx,
			`SELECT id, enterprise_capability_id, pillar_id, pillar_name, importance, rationale, set_at, updated_at
			 FROM architecturedirection.enterprise_strategic_importance
