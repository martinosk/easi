...
func (rm *BusinessDomainNameCacheReadModel) Upsert(ctx context.Context, businessDomainID, name string) error {
	return rm.execForTenant(ctx,
		`INSERT INTO architecturedirection.business_domain_name_cache (tenant_id, business_domain_id, name)
		 VALUES ($1, $2, $3)
...
func (rm *BusinessDomainNameCacheReadModel) Delete(ctx context.Context, businessDomainID string) error {
	return rm.execForTenant(ctx,
		"DELETE FROM architecturedirection.business_domain_name_cache WHERE tenant_id = $1 AND business_domain_id = $2",
		businessDomainID,
	)
}
...
func (rm *BusinessDomainNameCacheReadModel) Name(ctx context.Context, businessDomainID string) (string, error) {
	...
	err = rm.db.WithReadOnlyTx(ctx, func(tx *sql.Tx) error {
		err := tx.QueryRowContext(ctx,
			`SELECT name FROM architecturedirection.business_domain_name_cache WHERE tenant_id = $1 AND business_domain_id = $2`,
