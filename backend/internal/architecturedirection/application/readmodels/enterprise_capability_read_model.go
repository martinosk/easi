...
func (rm *EnterpriseCapabilityReadModel) Insert(ctx context.Context, dto EnterpriseCapabilityDTO) error {
	err := rm.execByID(ctx,
		"DELETE FROM architecturedirection.enterprise_capabilities WHERE tenant_id = $1 AND id = $2",
		dto.ID,
	)
	if err != nil {
		return err
	...
	return rm.execTenantQuery(ctx,
		`INSERT INTO architecturedirection.enterprise_capabilities
...
func (rm *EnterpriseCapabilityReadModel) Update(ctx context.Context, params UpdateCapabilityParams) error {
	return rm.execTenantQuery(ctx,
		`UPDATE architecturedirection.enterprise_capabilities SET name = $1, description = $2, category = $3, updated_at = CURRENT_TIMESTAMP
...
func (rm *EnterpriseCapabilityReadModel) Delete(ctx context.Context, id string) error {
	return rm.execByID(ctx, "UPDATE architecturedirection.enterprise_capabilities SET active = false, updated_at = CURRENT_TIMESTAMP WHERE tenant_id = $1 AND id = $2", id)
}
...
func (rm *EnterpriseCapabilityReadModel) ActiveEnterpriseCapabilityNames(ctx context.Context) (map[string]string, error) {
	...
	err = rm.db.WithReadOnlyTx(ctx, func(tx *sql.Tx) error {
		rows, err := tx.QueryContext(ctx,
			"SELECT id, name FROM architecturedirection.enterprise_capabilities WHERE tenant_id = $1 AND active = true",
			tenantID.Value(),
		)
		if err != nil {
			return err
...
func (rm *EnterpriseCapabilityReadModel) Count(ctx context.Context) (int, error) {
	...
	err = rm.db.WithReadOnlyTx(ctx, func(tx *sql.Tx) error {
		return tx.QueryRowContext(ctx,
			"SELECT COUNT(*) FROM architecturedirection.enterprise_capabilities WHERE tenant_id = $1",
			tenantID.Value(),
		).Scan(&count)
	})
	return count, err
...
func (rm *EnterpriseCapabilityReadModel) GetByID(ctx context.Context, id string) (*EnterpriseCapabilityDTO, error) {
	...
	err = rm.db.WithReadOnlyTx(ctx, func(tx *sql.Tx) error {
		dto, scanErr := scanEnterpriseCapability(tx.QueryRowContext(ctx,
			"SELECT "+ecSelectColumns+" FROM architecturedirection.enterprise_capabilities WHERE tenant_id = $1 AND id = $2",
			tenantID.Value(), id,
		))
...
const ecActiveSelect = "SELECT " + ecSelectColumns + " FROM architecturedirection.enterprise_capabilities WHERE tenant_id = $1 AND active = true"
...
func (rm *EnterpriseCapabilityReadModel) NameExists(ctx context.Context, name, excludeID string) (bool, error) {
	...
	err = rm.db.WithReadOnlyTx(ctx, func(tx *sql.Tx) error {
		if excludeID != "" {
			return tx.QueryRowContext(ctx,
				"SELECT COUNT(*) FROM architecturedirection.enterprise_capabilities WHERE tenant_id = $1 AND LOWER(name) = LOWER($2) AND id != $3 AND active = true",
				tenantID.Value(), strings.TrimSpace(name), excludeID,
			).Scan(&count)
		}
		return tx.QueryRowContext(ctx,
			"SELECT COUNT(*) FROM architecturedirection.enterprise_capabilities WHERE tenant_id = $1 AND LOWER(name) = LOWER($2) AND active = true",
...
func (rm *EnterpriseCapabilityReadModel) UpdateTargetMaturity(ctx context.Context, id string, targetMaturity int) error {
	return rm.execTenantQuery(ctx,
		`UPDATE architecturedirection.enterprise_capabilities SET target_maturity = $1, updated_at = CURRENT_TIMESTAMP
