...
- `application/readmodels/` — enterprise capability, strategic importance, TIME suggestion, direction, standard application, TIME assessment, realisation role, journey read models; the TIME assessment view that composes suggestions into assessment reads; capability node cache; pillar / realisation / importance / fit / business-domain-name caches; maturity analysis
...
- Endpoints: `/enterprise-capabilities` (CRUD, target maturity, strategic importance, direction, standard application, composition, maturity gap), `/enterprise-capability-compositions`, `/time-assessments`, `/realization-roles`, `/capability-journeys`

### Persistence
Schema `architecturedirection`; tables include `enterprise_capabilities`, `enterprise_strategic_importance`, `directions`, `direction_source_capabilities`, `standard_applications`, `time_assessments`, `realization_roles`, `capability_journeys`, `reference_name_cache`, `capability_domain_cache`, `capability_node_cache`, `realization_cache`, `business_domain_name_cache`, `ea_importance_cache`, `ea_fit_score_cache`, `ea_strategy_pillar_cache`. Migrations 137/138 add and backfill the spec-207 capability node cache; 145/146 add and backfill the realisation cache, reconcile the reference-name cache and make `maturity_value` non-null (spec 209); 150/151 re-parent the enterprisearchitecture tables into this schema, drop the now-redundant `enterprise_capability_cache` and re-seed the relocated backfills (spec 210); 152 drops `ea_realization_cache` and `domain_capability_metadata`, whose only reader was the TIME suggestion query, now served by `realization_cache`, `capability_node_cache` and `reference_name_cache` (spec 212). The legacy `link_count` / `domain_count` columns on `enterprise_capabilities` are unused since spec 207.
