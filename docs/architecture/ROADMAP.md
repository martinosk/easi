...
| H1-2 | Application record: ownership state machine with stats projection, hosting classification, two-level composition | SD6 | 214–216 | 214 done (ownership state machine Unknown → Nominated → Owned/Managed with typed owner references, HATEOAS affordances, statistics endpoint, team-deletion revert, AM user-name cache); 215 done (hosting classification); 216 done (per-tenant `ComponentContainments` aggregate, composition/aggregation kind, two-level cap, parent deletion cascades composed parts and releases aggregated parts, `x-attach-to`/`x-detach` affordances) |
| H1-3 | MetaModel: custom-field schema moves in (backfilled), modelling rule set v1 with AI naming validation | SD5 | 217–218 | 217 implemented, awaiting sign-off (per-subject-type `SubjectAttributeSchema` aggregate in MetaModel, OnePagers consumes its events into a backfilled definition cache and keeps only inclusion, order and required-ness; legacy definitions transferred with FieldIDs preserved — smell G1 closed); 218 pending approval |
