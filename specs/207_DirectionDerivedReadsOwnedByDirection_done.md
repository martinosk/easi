...
- [x] `GET /enterprise-capability-compositions` returns one summary per enterprise capability (`enterpriseCapabilityId`, `sourceCount`, `includedCount`, `domainCount`, `hasDirection`, `directionStatus`) under the same read permission as the composition view. (2026-08-31: field renamed from `hasActiveDirection` to `hasDirection` — it reports any non-rejected direction, not only an "active" one, and was unreleased.)
...
- **Capability node cache** — one row per domain capability: name, level, parent, L1 ancestor, effective business domain (id + name), maturity value. Domain names come from Architecture Direction's existing reference-name cache, which already tracks `BusinessDomainCreated/Updated`. (2026-08-31: on a hierarchy with no L1 in its ancestor chain, `l1AncestorOf` returns the topmost ancestor rather than the starting node returned by EA's `traverseToL1` — a deliberate, kept divergence, since it is the better behaviour for that anomalous case.)
