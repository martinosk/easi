...
10. **Milestone order was insertion order until spec 196** (decided during implementation) — the spec defines no reorder operation, so positions are implicit in the sequence of `JourneyMilestoneAdded` events and compact on removal. Spec 196 supersedes this decision with `JourneyMilestonesReordered`.
