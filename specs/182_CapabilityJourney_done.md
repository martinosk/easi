...
10. **Milestone order is insertion order** (decided during implementation) — the spec defines no reorder operation, so positions are implicit in the sequence of `JourneyMilestoneAdded` events and compact on removal. A reorder event can be added by a later spec if architects ask for it. Spec 196 added it (`JourneyMilestonesReordered`).
