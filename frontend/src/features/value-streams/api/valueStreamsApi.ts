...
export const valueStreamsApi = {
  async getAll(): Promise<ValueStreamsResponse> {
    const response = await httpClient.get<ValueStreamsResponse>(
  ...
  async getById(id: ValueStreamId): Promise<ValueStreamDetail> {
    const response = await httpClient.get<ValueStreamDetail>(
      `/api/v1/value-streams/${id}`,
    );
    return response.data;
  },

  async create(request: CreateValueStreamRequest): Promise<ValueStream> {
    const response = await httpClient.post<ValueStream>(
      "/api/v1/value-streams",
      request,
    );
    return response.data;
  ...
  async update(
    ...
    const response = await httpClient.put<ValueStream>(
      followLink(valueStream, "edit"),
      request,
    );
    return response.data;
  },
  ...
  ): Promise<ValueStreamDetail> {
    const response = await httpClient.post<ValueStreamDetail>(
      followLink(valueStream, "x-add-stage"),
      request,
    );
    return response.data;
  },
  ...
  async updateStage(
    ...
    const response = await httpClient.put<ValueStreamDetail>(
      followLink(stage, "edit"),
      request,
    );
    return response.data;
  },
  ...
  async reorderStages(
    ...
    const response = await httpClient.put<ValueStreamDetail>(
      followLink(valueStream, "x-reorder-stages"),
      request,
    );
    return response.data;
  },

  async addStageCapability(
    stage: ValueStreamStage,
    capabilityId: string,
  ): Promise<ValueStreamDetail> {
    const response = await httpClient.post<ValueStreamDetail>(
  ...
  async getByCapabilityId(
    capabilityId: string,
  ): Promise<CapabilityValueStreamsResponse> {
    const response = await httpClient.get<CapabilityValueStreamsResponse>(
      `/api/v1/capabilities/${capabilityId}/value-streams`,
    );
    return response.data;
  },
};
