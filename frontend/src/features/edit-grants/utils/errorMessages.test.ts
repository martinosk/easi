...
describe('getEditGrantErrorMessage', () => {
  ...
  it('should map duplicate grant error to user-facing message', () => {
    expect(getEditGrantErrorMessage('An active edit grant already exists for this user and artifact')).toBe(
  ...
  it('should map already expired error to user-facing message', () => {
    expect(getEditGrantErrorMessage('Edit grant has already expired')).toBe('This edit grant has already expired.');
  });

  it('should map not found error to user-facing message', () => {
    expect(getEditGrantErrorMessage('Edit grant not found')).toBe('The edit grant was not found.');
  });

  it('should map invalid artifact type error to user-facing message', () => {
    expect(getEditGrantErrorMessage('Invalid artifact type')).toBe('The artifact type is not valid.');
  });
