...
**Skills to consult for project-specific canonical patterns:** `easi-domain-driven-design` (aggregate boundaries, bounded contexts), `easi-database-migrations` (RLS, tenant isolation, database user separation), `easi-api-standards` (auth annotations, `@Security CookieAuth`, 401/403 documentation). Apply security analysis on top of these — do not redefine the patterns.
